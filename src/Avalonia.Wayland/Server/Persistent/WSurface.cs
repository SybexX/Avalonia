...
class WSurface : IPersistentWaylandObject, IWSurface
{
    protected readonly WaylandWorker _worker;
    protected WaylandConnection? _connection;
    protected WaylandGlobals? _globals;
    protected WlSurface? _wlSurface;
    protected WpFractionalScaleV1? _fractionalScale;
    protected WpViewport? _viewport;
    protected int? _lastPreferredBufferScale;
    protected double? _preferredFractionalScale;
    protected List<WaylandOutputsTracker.Output> _outputs = new();
    private WlCallback? _frameCallback;
    private double _currentScale = 1;
    private const double ScaleEpsilon = 1e-6;

    public WlSurface? WlSurface => _wlSurface;
    public WpViewport? Viewport => _viewport;
    public bool HasFractionalScaling => _fractionalScale != null && _viewport != null;
    public WaylandGlobals? Globals => _globals;
    public WlDisplay? CurrentDisplay => _connection?.Display;
    ...
    private WaylandTextInputV3? TextInputV3 => _globals?.InputDispatcher?.TextInputV3;

    /// <summary>
    /// Most recently requested cursor type. Lives on the worker thread —
    /// updated via <see cref="SetCursor"/>, consumed by the
    /// <see cref="WaylandInputDispatcher"/> when refreshing the pointer cursor.
    /// </summary>
    internal StandardCursorType CurrentCursor { get; private set; } = StandardCursorType.Arrow;

    public virtual void SetCursor(StandardCursorType cursorType)
    {
        CurrentCursor = cursorType;
        _globals?.InputDispatcher?.NotifyCursorChanged(this);
    }
    ...
    public void UiThreadWakeupRenderloop()
    {
        _worker.AnyThreadWakeupRenderLoop();
    }
    ...
    public virtual void OnConnected(WaylandConnection connection, WaylandGlobals globals)
    {
        ...
        _globals = globals;
        _wlSurface = _globals.WlCompositor.CreateSurface(new Listener(this));

        if (globals.HasFractionalScaling)
        {
            _fractionalScale = globals.FractionalScaleManager!.GetFractionalScale(
                _wlSurface, new FractionalScaleListener(this), connection.Queue);
    ...
    public IPlatformRenderSurface[] RenderSurfaces
    {
        get
        {
            var graphics = _worker.PlatformGraphics.CurrentGraphics;
    ...
    public virtual PlatformRenderTargetState State =>
        (Globals != null && _wlSurface != null && !IsFrameCallbackPending) ? PlatformRenderTargetState.Ready : default;
    ...
    {
        double newScale;
        if (_preferredFractionalScale is { } fractional)
        {
            newScale = fractional;
        }
        else if (_lastPreferredBufferScale is { } preferred)
        {
            newScale = preferred;
        }
        else
    ...
    public virtual void OnBeforeNewBufferAttached(IRenderTarget.RenderTargetSceneInfo sceneInfo)
    {
        if (HasFractionalScaling)
        {
            ...
            _viewport!.SetDestination(lw, lh);
        }
        else
            _wlSurface!.SetBufferScale(Math.Max(1, (int)Math.Ceiling(sceneInfo.Scaling)));

        _frameCallback = _wlSurface!.Frame(new FrameListener(this));
    }
    ...
    class FractionalScaleListener(WSurface p) : WpFractionalScaleV1.Listener
    {
        protected override void PreferredScale(WpFractionalScaleV1 eventSender, uint scale)
        {
            ...
            if (scale == 0)
                return;
            p._preferredFractionalScale = scale / 120.0;
            p.RecomputeScale();
        }
    }
    ...
    class Listener(WSurface p) : WlSurface.Listener
    {
        ...
        {
            p._lastPreferredBufferScale = factor;
            p.RecomputeScale();
        }

        protected override void Enter(WlSurface eventSender, WlOutput? output)
        {
            var o = p._globals?.Outputs.Outputs.FirstOrDefault(o => o.WlOutput == output);
            if (o != null)
                p._outputs.Add(o);
            p.OnOutputsChanged();
        }
    ...
    {
        _outputs.Clear();
        _lastPreferredBufferScale = null;
        _preferredFractionalScale = null;
...
class WXdgShellSurface : WSurface, IWXdgShellSurface
{
    ...
    /// <summary>
    /// Worker-thread accessor used by <see cref="WXdgPopup"/> when
    /// constructing the protocol popup against this parent.
    /// </summary>
    internal XdgSurface? XdgSurface => _xdgSurface;
    ...
    protected override void OnOutputsChanged()
    {
        ...
        var ids = _outputs.Count == 0
            ? (IReadOnlyList<object>)Array.Empty<object>()
            : _outputs.Select(o => o.Id).ToList();
    ...
    public override void OnConnected(WaylandConnection connection, WaylandGlobals globals)
    {
        ...
        _wlSurface!.Tags[typeof(WXdgShellSurface)] = this;
        _pendingAckSerial = null;
        _initialConfigureAcknowledged = false;
        _mapped = false;

        _xdgSurface = globals.XdgWmBase.GetXdgSurface(_wlSurface!, new XdgSurfaceListener(this));
    ...
    {
        _globals?.InputDispatcher?.TextInputV3?.UnregisterSurface(this);
        _wlSurface?.Tags.Remove(typeof(WXdgShellSurface));
        _xdgSurface?.Destroy();
        _xdgSurface = null;
        _pendingAckSerial = null;
        _initialConfigureAcknowledged = false;
    ...
    public override void OnBeforeNewBufferAttached(IRenderTarget.RenderTargetSceneInfo sceneInfo)
    {
        ...
        {
            _xdgSurface!.AckConfigure(_pendingAckSerial.Value);
            _pendingAckSerial = null;
        }
    ...
    protected virtual void MaybeEmitWindowGeometry(PixelSize bufferPixelSize, double bufferScale, Size logicalSize)
    {
        ...
        _xdgSurface!.SetWindowGeometry(left, top, right - left, bottom - top);
...
{
    protected TaskCompletionSource<XdgConfigureBatch> _basicInitCompletedTcs = new(TaskCreationOptions.RunContinuationsAsynchronously);
    private XdgToplevel? _xdgTopLevel;
    public Task<XdgConfigureBatch> BasicInitCompleted => _basicInitCompletedTcs.Task;
    ...
    {
        base.OnConnected(connection, globals);
        if (_globals!.Outputs.Outputs.Count == 0)
            throw new AvaloniaWaylandException("Expected at least one wl_output at this point");
        _pendingBatch = new();
        _xdgTopLevel = _xdgSurface!.GetToplevel(new TopLevelListener(this));
        if (!_csdSticky && _globals.XdgDecorationManager is { } decoMgr)
        {
    ...
    private Size CalculateMaxSize()
    {
        ...
        Size? maxSize = null;  
        var outputs = _outputs.Count > 0 ? _outputs : _globals!.Outputs.Outputs;
    ...
    protected override void OnConfigureBatchComplete(uint serial)
    {
        ...
        _basicInitCompletedTcs.TrySetResult(batch);
    ...
    private class DecorationListener(WXdgTopLevel p) : ZxdgToplevelDecorationV1.Listener
    {
        ...
        {
            // Translate the NWayland enum into our protocol-bindings-free
            // surface enum. Values 1 / 2 are stable per the v1 spec.
            ...
            if (p._basicInitCompletedTcs.Task.IsCompleted) // Only send events after initialization is complete
    ...
    {
        if (_globals?.XdgExporter is not { } exporter || _wlSurface == null || _xdgTopLevel == null
            || _connection == null)
            return null;
        var export = new XdgToplevelExport(this, _worker, exporter, _wlSurface, _connection.Queue);
...
class WXdgPopup : WXdgShellSurface, IWXdgPopup
{
    ...
    public void UpdatePositioner(XdgPopupPositionerParams positioner)
    {
        ...
        if (_xdgPopup == null || _connection == null)
            return;
    ...
    internal void TryAttachToParent()
    {
        ...
        if (_xdgSurface == null) return;                       // we're not connected
        ...
        {
            _pendingBatch = new();
            _xdgPopup = _xdgSurface.GetPopup(parentXdgSurface, positioner, new PopupListener(this));
        }
        finally
        {
            positioner.Destroy();
    ...
    private XdgPositioner? BuildPositioner(XdgPopupPositionerParams p)
    {
        ...
        var positioner = _globals.XdgWmBase.CreatePositioner();
